...
data_studi_neu2 <- data_studi_neu1 %>%
  ...
  mutate(region = case_when(str_detect(.$region, "Deutschland")~ "Deutschland",
                            T~ .$region))%>%
...
data_studi_neu4 <- data_studi_neu3 %>%
  ...
  mutate(order= "general")


# Creating values for gender, since by and bw are NA
data_studi_neu2_1 <- data_studi_neu1%>%
  select(-indikator, -hochschulform)%>%
  rename(indikator = label) %>%
  mutate(region = case_when(str_detect(.$region, "Deutschland")~ "Deutschland",
                            T~ .$region))%>%
  filter(!Jahr %in% c("2018", "2019", "2020", "2021"))%>%
  filter(!region %in% c("Baden-Württemberg" ,"Bayern", "Deutschland"))%>%
  pivot_wider(names_from = region, values_from =wert)%>%
  mutate(across(c(8:ncol(.)), as.numeric))%>%
  mutate("Deutschland" = rowSums(select(.,c(8:ncol(.)))))%>%
  mutate("Ostdeutschland (inkl. Berlin)" = rowSums(select(., c("Berlin", "Thüringen", "Sachsen", "Sachsen-Anhalt", "Brandenburg", "Mecklenburg-Vorpommern") ), na.rm =T ))%>%
  mutate("Westdeutschland (o. Berlin)"= Deutschland - `Ostdeutschland (inkl. Berlin)`)%>%
  pivot_longer(c(8:ncol(.)), names_to ="region", values_to = "wert")%>%

  ## Missing aggregagted values
  pivot_wider(names_from = fachbereich, values_from = wert)%>%
  mutate(across(c("Ingenieurwissenschaften", "Mathematik_Naturwissenschaften", "Alle" ), as.numeric))%>%
  mutate("MINT (Gesamt)"= Ingenieurwissenschaften+Mathematik_Naturwissenschaften)%>%
  mutate("Nicht MINT"= Alle - `MINT (Gesamt)`)%>%
  ...
  ## Adding missing years from 2018 on
  data_studi_neu2_1_append <-  data_studi_neu4%>%
    filter(jahr %in% c("2018", "2019", "2020", "2021"))

  data_studi_neu2_1 <- data_studi_neu2_1 %>%
    bind_rows(data_studi_neu2_1_append)%>%
    mutate(order= "gender")

studierende0 <- bind_rows(data_studi_neu4,data_studi_neu2_1)

studierende <- bind_rows(data_studi_neu4)
...
master_faecher_output$fach <- gsub("MINT", "MINT (Gesamt)", master_faecher_output$fach )
# master_faecher_output$fach <- gsub("Nicht MINT", "Alle Nicht MINT-Fächer", master_faecher_output$fach )
...
master_faecher_output1 <- master_faecher_output %>%
  ...
  mutate(typ=case_when(fach %in% c("Alle Fächer", "Naturwissenschaften", "Ingenieurwissenschaften ohne Informatik",
                                   "MINT (Gesamt)", "Nicht MINT")  ~ "Aggregat",
                       T ~ "Einzelauswahl"))
