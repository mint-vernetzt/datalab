...
mod_studium_studienzahl_all_23_ui <- function(id){
  ns <- NS(id)
  tagList(
    p("Auswahl des Jahres:"),
    shinyWidgets::sliderTextInput(
      inputId = ns("all_23_year"),
      label = NULL,
...
mod_studium_studienzahl_all_23_server <- function(id, r){
  moduleServer( id, function(input, output, session){
    ...
    observeEvent(input$all_23_year, {
      r$all_23_year <- input$all_23_year
    })

    observeEvent(input$all_23_label_sel, {
      r$all_23_label_sel <- input$all_23_label_sel
    })
